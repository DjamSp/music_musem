...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const historyHeaders2 = document.querySelectorAll('.dif h2');
    ...
    const articleCards = document.querySelectorAll('.card .text-word');
    ...
    const ruTexts = {
        ...
        // Секция истории - все абзацы (по порядку как в HTML)
        historyParagraphs: [
        ...
        nextBtn: 'дальше',
        articleWords: [
            'Влияет ли', 'музыка', 'на сюжет', 'в кино',
            'Как', 'музыка', 'влияет', 'на нас',
            'Как', 'современная', 'музыка', 'влияет', 'на', 'моду'
        ],
    ...
    // Тексты на английском (оригинальные из вашего HTML)
    const enTexts = {
        // Основной экран
        ...
        historySubtitles: [
            ...
            '(the second half of the XVIII<br>— the beginning of the XIX century)',
            '(19th century)',
            ''
        ],
        historyHeaders2: [
        ...
        // Секция истории - все абзацы (оригинальные английские)
        historyParagraphs: [
        ...
        nextBtn: 'farther',
        articleWords: [
            'Does', 'music', 'influence', 'the plot', 'in movies',
            'How', 'music', 'effects', 'us',
            'How', 'modern', 'music', 'influence', 'fashion'
        ],
    ...
    function translateToRussian() {
        ...
        if (historySubtitles.length > 0) {
            historySubtitles.forEach((el, index) => {
                if (ruTexts.historySubtitles[index]) {
                    el.innerHTML = ruTexts.historySubtitles[index];
                }
            });
        }
        
        if (historyHeaders2.length > 0) {
            historyHeaders2.forEach((el, index) => {
                if (ruTexts.historyHeaders2[index]) {
                    el.textContent = ruTexts.historyHeaders2[index];
                }
            });
        }
        
        // Секция истории - абзацы
        if (historyParagraphs.length > 0) {
            historyParagraphs.forEach((el, index) => {
                if (ruTexts.historyParagraphs[index]) {
                    el.textContent = ruTexts.historyParagraphs[index];
                }
            });
        }
        ...
        if (countrySubtitles.length > 0) {
            countrySubtitles.forEach((el, index) => {
                if (ruTexts.countrySubtitles[index]) {
                    el.textContent = ruTexts.countrySubtitles[index];
                }
            });
        }
        ...
        if (searchButton) searchButton.innerHTML = ruTexts.searchButton;
        
        // Секция артефактов
        if (artifactsHeader) artifactsHeader.innerHTML = ruTexts.artifactsHeader;
        
        if (artifactTitles.length > 0) {
            artifactTitles.forEach((el, index) => {
                if (ruTexts.artifactTitles[index]) {
                    el.textContent = ruTexts.artifactTitles[index];
                }
            });
        }
        
        if (artifactDescriptions.length > 0) {
            artifactDescriptions.forEach((el, index) => {
                if (ruTexts.artifactDescriptions[index]) {
                    el.textContent = ruTexts.artifactDescriptions[index];
                }
            });
        }
        ...
        if (prevBtn) prevBtn.innerHTML = ruTexts.prevBtn;
        if (nextBtn) nextBtn.innerHTML = ruTexts.nextBtn;
        
        if (articleCards.length > 0) {
            articleCards.forEach((el, index) => {
                if (ruTexts.articleWords[index]) {
                    el.textContent = ruTexts.articleWords[index];
                }
            });
        }
        ...
        if (footerLinks.length > 0) {
            footerLinks.forEach((el, index) => {
                if (ruTexts.footerLinks[index]) {
                    el.textContent = ruTexts.footerLinks[index];
                }
            });
        }
        
        if (footerInput) footerInput.placeholder = ruTexts.footerPlaceholder;
        if (footerButton) footerButton.innerHTML = ruTexts.footerButton;
    }
    ...
    function translateToEnglish() {
        ...
        if (historySubtitles.length > 0) {
            historySubtitles.forEach((el, index) => {
                if (enTexts.historySubtitles[index]) {
                    el.innerHTML = enTexts.historySubtitles[index];
                }
            });
        }
        ...
        if (historyHeaders2.length > 0) {
            historyHeaders2.forEach((el, index) => {
                if (enTexts.historyHeaders2[index]) {
                    el.textContent = enTexts.historyHeaders2[index];
                }
            });
        }
        ...
        if (historyParagraphs.length > 0) {
            historyParagraphs.forEach((el, index) => {
                if (enTexts.historyParagraphs[index]) {
                    el.textContent = enTexts.historyParagraphs[index];
                }
            });
        }
        ...
        if (countrySubtitles.length > 0) {
            countrySubtitles.forEach((el, index) => {
                if (enTexts.countrySubtitles[index]) {
                    el.textContent = enTexts.countrySubtitles[index];
                }
            });
        }
        ...
        if (searchButton) searchButton.innerHTML = enTexts.searchButton;
        ...
        if (artifactTitles.length > 0) {
            artifactTitles.forEach((el, index) => {
                if (enTexts.artifactTitles[index]) {
                    el.textContent = enTexts.artifactTitles[index];
                }
            });
        }
        ...
        if (artifactDescriptions.length > 0) {
            artifactDescriptions.forEach((el, index) => {
                if (enTexts.artifactDescriptions[index]) {
                    el.textContent = enTexts.artifactDescriptions[index];
                }
            });
        }
        ...
        if (prevBtn) prevBtn.innerHTML = enTexts.prevBtn;
        if (nextBtn) nextBtn.innerHTML = enTexts.nextBtn;
        
        if (articleCards.length > 0) {
            articleCards.forEach((el, index) => {
                if (enTexts.articleWords[index]) {
                    el.textContent = enTexts.articleWords[index];
                }
            });
        }
        ...
        if (footerLinks.length > 0) {
            footerLinks.forEach((el, index) => {
                if (enTexts.footerLinks[index]) {
                    el.textContent = enTexts.footerLinks[index];
                }
            });
        }
        
        if (footerInput) footerInput.placeholder = enTexts.footerPlaceholder;
        if (footerButton) footerButton.innerHTML = enTexts.footerButton;
    }
